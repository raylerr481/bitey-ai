...
class Bitey_API {
    ...
    private function normalize_language($value) {
        $value=strtolower(trim((string)$value)); $map=array('auto'=>'auto','pt'=>'pt-BR','pt-br'=>'pt-BR','es'=>'es','en'=>'en');
    ...
    private function call_backend($data) {
        ...
        $response=wp_remote_post($this->backend_url().'/chat',array('timeout'=>30,'headers'=>array('Accept'=>'application/json','Content-Type'=>'application/json'),'body'=>wp_json_encode(array(
            'message'=>$data['message'],'phone'=>$data['phone'],'email'=>$data['email'],'company_id'=>$data['company_id']?:1,'customer_name'=>trim($data['name'].' '.$data['last_name'])?:'Customer','channel'=>$data['channel']?:'website','conversation_id'=>$data['conversation_id'],'language_preference'=>$data['language_preference'],'preferred_contact_channel'=>$data['preferred_contact_channel'],
        ))));
        $body=$this->parse_backend_response($response); if(is_wp_error($body)) return $body;
    ...
    public function rest_chat(WP_REST_Request $request) {
        $data=$this->request_data($request->get_json_params()?:array()); $result=$this->call_backend($data);
    ...
    private function validate_upload($file) {
        ...
        if(!in_array($ext,$this->allowed_extensions,true)) return new WP_Error('file_type_not_allowed','Tipo de documento no permitido. Usa PDF, DOC, DOCX, TXT, CSV, JSON o MD.');
        ...
        return array('name'=>$name,'ext'=>$ext,'type'=>sanitize_mime_type($file['type']??'application/octet-stream'),'data'=>file_get_contents($file['tmp_name']));
    }
    ...
    public function import_company() {
        ...
        $file=$this->validate_upload($_FILES['company_document']??null); if(is_wp_error($file)) $this->fail($file->get_error_code(),$file->get_error_message(),400);
        $company_id=absint($_POST['company_id']??get_option('bitey_company_id',1))?:1; $company_name=sanitize_text_field(wp_unslash($_POST['company_name']??''));
        $response=$this->multipart_request($this->backend_url().'/company-profile/import',array('company_id'=>$company_id,'company_name'=>$company_name,'source'=>'wordpress_globe','channel'=>'website'),$file);
        $body=$this->parse_backend_response($response); if(is_wp_error($body)) $this->fail($body->get_error_code(),$body->get_error_message(),502,$body->get_error_data()?:array());
        wp_send_json_success($body);
    }
    public function send_message() {
        if(!check_ajax_referer('bitey_nonce','nonce',false)) $this->fail('invalid_nonce','Solicitud no autorizada.',403);
        $result=$this->call_backend($this->request_data($_POST)); if(is_wp_error($result)) $this->fail($result->get_error_code(),$result->get_error_message(),502,$result->get_error_data()?:array()); wp_send_json_success($result);
    }
}
