...
/**
 * Channel-neutral server bridge between WordPress and the Bitey API.
 * AJAX remains supported, while REST is the primary transport because some
 * WordPress hosting layers return the normal page HTML for admin-ajax.php.
 */
class Bitey_API
{
    ...
    {
        if ($data['message'] === '') {
            return new WP_Error('empty_message', 'Escribe un mensaje para continuar.');
        }

        $backend_url = untrailingslashit((string) get_option('bitey_backend_url', 'https://bitefixes-backend.onrender.com'));
        $response = wp_remote_post($backend_url . '/chat', array(
            'timeout' => 30,
            ...
            'body' => wp_json_encode(array(
                ...
                'customer_name'=>trim($data['name'].' '.$data['last_name']) ?: 'Customer',
        ...
        if ($status < 200 || $status >= 300) return new WP_Error('backend_http_error', 'Bitey Backend devolvió un error.', array('status'=>$status));
        ...
        return array(
            ...
            'customer_name'=>$body['customer_name'] ?? ($data['name'] ?: null),
            'language'=>$body['language'] ?? null,
            'language_source'=>$body['language_source'] ?? null,
            'conversation_id'=>$body['conversation_id'] ?? $data['conversation_id'],
            'preferred_contact_channel'=>$body['preferred_contact_channel'] ?? $data['preferred_contact_channel'],
    ...
    public function rest_chat(WP_REST_Request $request)
    {
        ...
        if (is_wp_error($result)) {
            $status = $result->get_error_code() === 'empty_message' ? 400 : 502;
            return new WP_REST_Response(array('success'=>false,'data'=>array('code'=>$result->get_error_code(),'reply'=>$result->get_error_message())), $status);
        }
    ...
    public function send_message()
    {
        ...
        $data = $this->request_data($_POST);
        $result = $this->call_backend($data);
        if (is_wp_error($result)) {
            $status = $result->get_error_code() === 'empty_message' ? 400 : 502;
            $this->fail($result->get_error_code(), $result->get_error_message(), $status, $result->get_error_data() ?: array());
        }
