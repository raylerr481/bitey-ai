...
class Bitey_API
{
    ...
    public function send_message()
    {
        if (!check_ajax_referer('bitey_nonce', 'nonce', false)) {
            $this->fail('invalid_nonce', 'Solicitud no autorizada.', 403);
        }
        ...
        if ($message === '') {
            $this->fail('empty_message', 'Escribe un mensaje para continuar.', 400);
        }
        ...
        if (is_wp_error($response)) {
            $this->fail('backend_unreachable', 'No se pudo conectar con Bitey Backend. Inténtalo nuevamente.', 502, array(
        ...
        if ($status < 200 || $status >= 300) {
            $this->fail('backend_http_error', 'Bitey Backend devolvió un error.', 502, array(
                'backend_status' => $status,
            ));
        }

        if (!is_array($body)) {
            $this->fail('invalid_backend_json', 'Bitey Backend respondió con datos no válidos.', 502);
        }
        ...
        if ($reply === '') {
            $this->fail('empty_backend_response', 'Bitey recibió tu mensaje, pero no generó una respuesta.', 502);
        }
