...
class Bitey_API {
    ...
    private function call_backend($data) {
        ...
        $response=wp_remote_post($this->backend_url().'/chat',array(
            'timeout'=>45,
            'headers'=>$headers,
    ...
    private function validate_upload($file) {
        ...
        $mime=sanitize_text_field($file['type']??'application/octet-stream');
        return array('name'=>$name,'ext'=>$ext,'type'=>$mime,'data'=>file_get_contents($file['tmp_name']));
    }
    ...
    public function import_company() {
        if(!current_user_can('manage_options')) $this->fail('forbidden','Solicitud no autorizada.',403);
        ...
        if($this->channel_key()==='') $this->fail('channel_key_missing','Configura la Channel API Key de Bitey antes de importar documentos.',503);
        $response=$this->multipart_request($this->backend_url().'/company-profile/import',array('company_id'=>$company_id,'company_name'=>$company_name,'source'=>'wordpress','channel'=>'website'),$file);
        $body=$this->parse_backend_response($response);
