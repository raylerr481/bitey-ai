(function () {
    ...
    function initBitey() {
        ...
        if (!button || !win || !send || !input || !messages || !lang) return;
        if (!window.bitey_ajax || !window.bitey_ajax.ajax_url || !window.bitey_ajax.nonce) return;
        window.__biteyInitialized=true;
        const KEY='bitey_session_v3';
        let state={identified:false,name:'',phone:'',language:'auto',detected_language:'',conversation_id:'',customer_id:''};
        ...
        function detectedOrDefault(){return ['es','pt-BR','en'].includes(state.detected_language)?state.detected_language:'es';}
        function uiLanguage(){return state.language==='auto'?detectedOrDefault():(labels[state.language]?state.language:'es');}
        ...
        function syncIdentity(){dataBox.style.display=(state.identified&&state.name&&state.phone)?'none':'flex';}
        syncIdentity();lang.value=state.language||'auto';refreshUI();
        ...
        identify.addEventListener('click',()=>{const n=name.value.trim(),p=phone.value.trim();if(!n||!p){if(!n)name.focus();else phone.focus();return;}state.identified=true;state.name=n;state.phone=p;save();syncIdentity();welcome.textContent=labels[uiLanguage()].welcome(n);input.focus();});
        ...
        function sendMessage(){const message=input.value.trim();if(!message||send.disabled)return;if(!state.identified){syncIdentity();name.focus();return;}addMessage(message,'bitey-user');input.value='';send.disabled=true;const typing=addMessage(labels[uiLanguage()].typing,'bitey-ai');const form=new FormData();form.append('action','bitey_send_message');form.append('message',message);form.append('nonce',window.bitey_ajax.nonce);form.append('name',state.name);form.append('phone',state.phone);form.append('company_id',window.bitey_ajax.company_id||1);form.append('channel',window.bitey_ajax.channel||'website');form.append('conversation_id',state.conversation_id||'');form.append('language_preference',state.language||'auto');fetch(window.bitey_ajax.ajax_url,{method:'POST',body:form,credentials:'same-origin'}).then(r=>{if(!r.ok)throw Error('HTTP '+r.status);return r.json();}).then(d=>{typing.remove();const x=d&&d.data?d.data:{};if(x.conversation_id)state.conversation_id=x.conversation_id;if(x.customer_id)state.customer_id=x.customer_id;if(x.customer_name)state.name=x.customer_name;if(x.language)state.detected_language=x.language;save();refreshUI();addMessage(x.reply||labels[uiLanguage()].error,'bitey-ai');}).catch(()=>{typing.remove();addMessage(labels[uiLanguage()].error,'bitey-ai');}).finally(()=>{send.disabled=false;input.focus();});}
    }
