(function () {
    ...
    function initBitey() {
        ...
        const KEY = 'bitey_session_v4';
        let state = { name: '', phone: '', language: 'auto', detected_language: '', conversation_id: '', customer_id: '' };
        try { state = Object.assign(state, JSON.parse(localStorage.getItem(KEY) || '{}')); } catch (e) {}

        const ui = {
            es: { welcome: 'Hola 👋 soy Bitey. ¿Cómo puedo ayudarte?', input: 'Escribe tu mensaje...', typing: 'Bitey está pensando...', error: 'No pude conectar con Bitey. Inténtalo nuevamente.' },
            'pt-BR': { welcome: 'Olá 👋 sou o Bitey. Como posso ajudar?', input: 'Digite sua mensagem...', typing: 'Bitey está pensando...', error: 'Não consegui conectar ao Bitey. Tente novamente.' },
            en: { welcome: "Hello 👋 I'm Bitey. How can I help?", input: 'Type your message...', typing: 'Bitey is thinking...', error: 'I could not connect to Bitey. Please try again.' }
        };

        function save() { try { localStorage.setItem(KEY, JSON.stringify(state)); } catch (e) {} }
        function language() { return ui[state.detected_language] ? state.detected_language : 'es'; }
        function refresh() { const l = ui[language()]; input.placeholder = l.input; if (!messages.querySelector('.bitey-user')) welcome.textContent = l.welcome; }
        refresh();
        ...
        function sendMessage() {
            ...
            input.value = '';
            send.disabled = true;
            const typing = addMessage(ui[language()].typing, 'bitey-ai');
            ...
            fetch(window.bitey_ajax.ajax_url, { method: 'POST', body: form, credentials: 'same-origin' })
                ...
                .then(d => {
                    ...
                    save();
                    refresh();
                    addMessage(x.reply || ui[language()].error, 'bitey-ai');
                })
                .catch(() => { typing.remove(); addMessage(ui[language()].error, 'bitey-ai'); })
